...
/**
 * Crawler Constructor:
 * Build a tree from an URL, exploring iteratively among children and saving it to MongoDB
 * @param url
 * @constructor
 */
class Crawler {
  ...
  async processUrls (node, level, toplevel, conf) {
    try {
      ...
      if (DOM) {
        ...
        let children = this.addUrlsToNode(this.tree.getDatosNode(node), links)
